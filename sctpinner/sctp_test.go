...
var resolveSCTPAddrTests = []resolveSCTPAddrTest{
	{"sctpinner", "127.0.0.1:0", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.IPv4(127, 0, 0, 1)}}, Port: 0}, nil},
	{"sctp4", "127.0.0.1:65535", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.IPv4(127, 0, 0, 1)}}, Port: 65535}, nil},

	{"sctpinner", "[::1]:0", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.ParseIP("::1")}}, Port: 0}, nil},
	{"sctp6", "[::1]:65535", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.ParseIP("::1")}}, Port: 65535}, nil},

	{"sctpinner", "[fe80::1%eth0]:0", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.ParseIP("fe80::1"), Zone: "eth0"}}, Port: 0}, nil},
	{"sctp6", "[fe80::1%eth0]:65535", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.ParseIP("fe80::1"), Zone: "eth0"}}, Port: 65535}, nil},

	{"sctpinner", ":12345", &SCTPAddr{Port: 12345}, nil},

	{"sctpinner", "127.0.0.1/10.0.0.1:0", &SCTPAddr{IPAddrs: []net.IPAddr{net.IPAddr{IP: net.IPv4(127, 0, 0, 1)}, net.IPAddr{IP: net.IPv4(10, 0, 0, 1)}}, Port: 0}, nil},
...
func TestSCTPConcurrentAccept(t *testing.T) {
	...
	addr, _ := ResolveSCTPAddr("sctpinner", "127.0.0.1:0")
	ln, err := ListenSCTP("sctpinner", addr)
	if err != nil {
		t.Fatal(err)
	}
	const N = 10
	...
	for i := 0; i < attempts; i++ {
		c, err := DialSCTP("sctpinner", nil, ln.Addr().(*SCTPAddr))
		if err != nil {
			fails++
		} else {
			c.Close()
...
func TestSCTPCloseRecv(t *testing.T) {
	...
	_, err = DialSCTP("sctpinner", nil, ln.Addr().(*SCTPAddr))
	if err != nil {
